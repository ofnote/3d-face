...
def show_landmarks(img,landmarks):
    ...
    img = cv2.cvtColor(img,cv2.COLOR_BGR2RGB)
    landmarks = np.array(landmarks.cpu())
    # print(img.shape)

    for i in range(n):
        # if i in [37,38,40,41,43,44,46,47]:
        #     cv2.circle(img,(int(landmarks[i][0]),int(landmarks[i][1])),1,(0,0,255),1)            
        # else:
        cv2.circle(img,(int(landmarks[i][0]),int(landmarks[i][1])),1,(255,255,0),1)
    cv2.imshow("image",img)
    cv2.waitKey(0)

def show_comp_landmarks(img,landmarks,pred):
    n = landmarks.shape[0]
    ...
    for i in range(n):
        ...
        #     cv2.circle(img,(int(landmarks[i][0]),int(landmarks[i][1])),1,(0,0,255),1)            
        # else:
        cv2.circle(img,(int(landmarks[i][0]),int(landmarks[i][1])),1,(255,255,0),2)
        cv2.circle(img,(int(pred[i][0]),int(pred[i][1])),1,(0,0,255),2)

    cv2.imshow("image",img)

    cv2.waitKey(0)

#-----------------Calculating Feature Embedding-------------#
def featureEmbedding(img):
    model = InceptionResnetV1(pretrained='vggface2').eval()
    
    # print(img.shape)
    
    img_embedding = model(img.view(1,3,224,224).detach().cpu())
    
    # print('------------------Printing Image Embedding---------------')
    # print(img_embedding)
